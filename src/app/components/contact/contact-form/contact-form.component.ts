...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, MatInputModule, MatButtonModule, FormularioDebugComponent, TranslateModule],
...
export class ContactFormComponent {
  ...
  private sendEmail() {
    this.contactService.sendEmail(this.form.value)
      ...
      .subscribe({
        next: () => {
          this.matSnackBarService.success(this.translateService.instant('contact_form.message.success'), this.translateService.instant('generic_messages.action_close'), 5000);
          this.form.reset();
        },
